...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.dataset_mode == 'pannuke128':
            if self.is_train:
                if self.random_crop:
...
def random_crop_arr(pil_list, image_size, min_crop_frac=0.8, max_crop_frac=1.0):
    min_smaller_dim_size = math.ceil(image_size / max_crop_frac) # 128
    max_smaller_dim_size = math.ceil(image_size / min_crop_frac) # 128 / 0.5 = 256
